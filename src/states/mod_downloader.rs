...
use iced::{
    Color, Element, Radians, Task,
    advanced::{image::Handle as RasterHandle, svg::Handle as SvgHandle},
    gradient::Linear,
    widget::{
...
use crate::{
    ImageType, STATIC_IMAGES,
    Message::{self, ModDLMessage as SuperMsg},
    reqwests,
};
    ...
    pub fn update(&mut self, _message: ModDownMsg) -> Task<Message> {
        match _message {
            ...
            ModDownMsg::ModListingPressed(index) => {
                ...
                if self.mods_cached.contains_key(slug) {
                    return self.spotlight_mod(&slug.clone());
                } else {
                    self.markup_state = MarkState::with_html("loading...");
                    return Task::batch([
                        Task::perform(reqwests::fetch_mod(slug.clone()), ModDownMsg::ModReceived),
                        task,
                    ])
                    .map(|m| SuperMsg(m));
                }
            }
            ModDownMsg::ModReceived(res) => match res {
                Ok(val) => {
                    let des = &mut serde_json::Deserializer::from_slice(&val);
                    let result: Result<ModrinthMod, _> = serde_path_to_error::deserialize(des);
                    match result {
                        Ok(m) => {
                            if self.selected_mod == m.slug {
                                let slug = m.slug.clone();
                                self.mods_cached.insert(m.slug.clone(), m);
                                return self.spotlight_mod(&slug);
                            }
                            self.mods_cached.insert(m.slug.clone(), m);
                        }
                        Err(e) => {
                    ...
                    if !self.scroll_load_debounce {
                        ...
                        return Task::perform(
                            reqwests::search_mods(
                                ...
                                None,
                                Some(vec!["server_side!=unsupported".to_string()]),
                            ),
            ...
            ModDownMsg::SearchSubmitted => {
                ...
                return self
                    ._new_mod_search(Some(vec!["server_side!=unsupported".to_string()]))
                    .map(|m| SuperMsg(m));
            }
        };
        Task::none()
                    ...
                    if !self.selected_mod.is_empty() && let Some(listing) = self.mods_cached.get(&self.selected_mod) {
                        ...
                        let thumbnail: Element<ModDownMsg> = if let Some(url) = &listing.icon_url {
                            if let Some(img) = self.cached_images.get(url).cloned() {
                                ...
                                widget::image(&STATIC_IMAGES.missing).width(IMG_SIZE).height(IMG_SIZE).into()
                            }
                        } else {
                            eprintln!("nonexistent thumbnail in search builder");
                            widget::Image::new(&STATIC_IMAGES.missing).width(IMG_SIZE).height(IMG_SIZE).into()
                        };
                            ...
                            MarkWidget::new(&self.markup_state).on_clicking_link(|url| {ModDownMsg::OpenLink(url)})
                                .on_drawing_image(|info| {
                                    if let Some(image) = self.cached_images.get(info.url).cloned() {
                                        ...
                                        widget::image(&STATIC_IMAGES.missing).width(128).height(128).into()
                                    }
                                }
                            )
                        ].spacing(10).into()
                    } else {Element::from("")}
                ).width(iced::Fill).spacing(5).id(widget::Id::new("markup"))
            ].spacing(10)
        ].spacing(10).padding(15).into()
    }

    pub fn init(&mut self) -> Task<Message> {
        self._new_mod_search(Some(vec!["server_side!=unsupported".to_string()]))
            .map(|m| SuperMsg(m))
    }

    pub fn spotlight_mod(&mut self, slug: &str) -> Task<Message> {
        let mod_data = self.mods_cached.get(slug).unwrap();
