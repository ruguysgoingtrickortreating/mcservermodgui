...
use iced::{Alignment, Color, Degrees, Element, Gradient, Radians, Task, advanced::{image::Handle as RasterHandle, svg::Handle as SvgHandle, widget::operation}, color, gradient::Linear, widget::{self, Button, button, column, container, row, scrollable::{self, Viewport}, text, text_input}};
...
fn main() -> iced::Result {
    iced::application("mcservermodgui",App::update,App::view)
    .theme(|_| iced::Theme::Custom(Arc::new(iced::theme::Custom::new("".to_string(), iced::theme::Palette {
        primary: Color::TRANSPARENT,
        ..iced::theme::Palette::DARK
    }))))
    .run_with(|| {
        let mut app = App::default();
        let t = app._new_mod_search(Some(vec!["server_side!=unsupported".to_string()]));
        (app,t)
    })
}
...
enum Message {
    ...
    LinkOpen(String),
    ImageDownloaded(Result<ImageData, String>),
...
impl App {
    fn update(&mut self, _message:Message) -> Task<Message> {
        match _message {
            Message::None => (),
            Message::LinkOpen(url) => {let _ = webbrowser::open(url.as_str());},
            ...
            Message::ModsSearchReceived(res) => match res{
                Ok(val) => {
                        ...
                        Ok(search) => {
                            ...
                            for r in search.hits {
                                if let Some(url) = &r.icon_url {
                                    img_tasks.push(Task::perform(reqwests::download_image(url.clone()),Message::ImageDownloaded));
                                }
            ...
            Message::ModListingPressed(index) => {
                ...
                if let Some(modrinth_mod) = self.mods_cached.get(slug) {
                    ...
                    return Task::batch([self._download_markup_images(),task]);
                } else {
                    self.markup_state = MarkState::with_html("loading...");
                    return Task::batch([Task::perform(reqwests::fetch_mod(slug.clone()), Message::ModRecieved),task]);
                }
            },
            Message::ModRecieved(res) => match res {
                Ok(val) => {
                    let des = &mut serde_json::Deserializer::from_slice(&val);
                    ...
                    match result {
                        Ok(m) => {
                            ...
                            if self.selected_mod == slug {
                                ...
                                return self._download_markup_images()
                            }
                            self.mods_cached.insert(slug,m);
                        }
                        Err(e) => {
            ...
            Message::ModsListScrolled(viewport) => {
                if viewport.absolute_offset_reversed().y <= 300.0 {
                    if !self.scroll_load_debounce {
                        ...
                        return Task::perform(reqwests::search_mods(self.mods_search_offset, None, Some(vec!["server_side!=unsupported".to_string()])),Message::ModsSearchReceived);
                    }
            ...
            Message::SearchSubmitted => {
                ...
                return self._new_mod_search(Some(vec!["server_side!=unsupported".to_string()]));
            }
        }
        Task::none()
    }

    fn _download_markup_images(&mut self) -> Task<Message> {
        Task::batch(self.markup_state.find_image_links().into_iter().map(|url| {
            if self.images_queued.insert(url.clone()) {
                Task::perform(reqwests::download_image(url), Message::ImageDownloaded)
            } else {
                Task::none()
            }
        }))
    ...
    fn _new_mod_search(&mut self, args: Option<Vec<String>>) -> Task<Message> {
        self.is_search_fetching = true;
        self.mods_search_results.clear();
        self.mods_search_offset = 0;
        println!("{:?}",self.mods_search_results);
        Task::batch([
            Task::perform(reqwests::search_mods(self.mods_search_offset, Some(self.current_query.clone()), args),Message::ModsSearchReceived),
            scrollable::scroll_to(scrollable::Id::new("search"), scrollable::AbsoluteOffset::default())])
    }
    
    fn _create_mod_listing(&'_ self, mods_list_index:usize) -> iced::Element<'_, Message> {
        const IMG_SIZE:u16 = 75;
        let listing = &self.mods_search_results[mods_list_index];
        let thumbnail: Element<Message> = if let Some(url) = &listing.icon_url {
            if let Some(img) = self.cached_images.get(url).cloned() {
                ...
                widget::image(&self.missing_image).width(IMG_SIZE).height(IMG_SIZE).into()
            }
        } else {
            eprintln!("nonexistent thumbnail in search builder");
            widget::Image::new(&self.missing_image).width(IMG_SIZE).height(IMG_SIZE).into()
        };
        ...
        let mut button = button(contents).on_press_with(move || Message::ModListingPressed(mods_list_index)).height(80).width(iced::Fill).padding([5, 0]);
    ...
    fn view(&'_ self) -> iced::Element<'_, Message> {
        column![
            text_input("Search...", &self.current_searchbar_text).on_input(|s| Message::SearchTyped(s)).on_submit(Message::SearchSubmitted).icon(text_input::Icon { font: iced::Font::DEFAULT, code_point: '⌕', size: None, spacing: 4.0, side: text_input::Side::Left }),
            row![
                ...
                ).width(320).spacing(5).on_scroll(Message::ModsListScrolled).id(scrollable::Id::new("search")),
                widget::scrollable( // markdown section
                    if !self.selected_mod.is_empty() && let Some(listing) = self.mods_cached.get(&self.selected_mod) {
                        ...
                        column![
                            ...
                            MarkWidget::new(&self.markup_state).on_clicking_link(|url| Message::LinkOpen(url))
                                .on_drawing_image(|info| {
                                    if let Some(image) = self.cached_images.get(info.url).cloned() {
                                        ...
                                        widget::image(&self.missing_image).width(128).height(128).into()
                                    }
                                }
                            )
                        ].spacing(10).into()
