...
impl<'a, Message, Theme> Widget<Message, Theme, Renderer> for Circular<Theme>
where
    Message: 'a + Clone,
    Theme: StyleSheet,
{
            ...
            match state.animation {
                Animation::Expanding { progress, .. } => {
                    builder.arc(canvas::path::Arc {
                        ...
                        radius: track_radius,
                        start_angle: start,
                        end_angle: start + MIN_ANGLE + WRAP_ANGLE * progress, // (self.easing.y_at_x(progress)),
                    });
                }
                Animation::Contracting { progress, .. } => {
                    builder.arc(canvas::path::Arc {
                        center: frame.center(),
                        radius: track_radius,
                        start_angle: start + WRAP_ANGLE * progress, // (self.easing.y_at_x(progress)),
