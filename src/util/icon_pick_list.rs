...
use iced::advanced::{layout, mouse, overlay, renderer, svg, text, Clipboard, Layout, Shell, Text, Widget};
use iced::{alignment, keyboard, touch, window, Background, Border, Color, Element, Event, Length, Padding, Pixels, Point, Rectangle, Size, Theme, Vector};
...
mod menu;
use menu::Menu;
use crate::STATIC_IMAGES;

pub fn icon_pick_list<'a, T, F, L, V, Message, Theme, Renderer>(
    options: L,
    selected: Option<V>,
    on_selected: impl Fn(T) -> Message + 'a,
    determining_method: F,
) -> IconPickList<'a, T, F, L, V, Message, Theme, Renderer>
where
...
{
    IconPickList::new(options, selected, on_selected, determining_method)
}

pub struct IconPickList<
    'a,
    T,
    F,
    L,
...
impl<'a, T, F, L, V, Message, Theme, Renderer>
IconPickList<'a, T, F, L, V, Message, Theme, Renderer>
where
    T: ToString + PartialEq + Clone,
    F: Fn(&T) -> Vec<Svg> + 'a,
...
impl<'a, T, F, L, V, Message, Theme, Renderer> Widget<Message, Theme, Renderer>
for IconPickList<'a, T, F, L, V, Message, Theme, Renderer>
where
    ...
    Renderer: text::Renderer + svg::Renderer + 'a,
{
    ...
    fn layout(
        ...
        let max_width = match self.width {
            Length::Shrink => {
                let labels_width =
                    state.options.iter().zip(self.options.borrow().iter()).fold(0.0, |width, (paragraph, T)| {
                        let icons_amount = (self.svgs_from_option)(T).len();
                        f32::max(width, paragraph.min_width() + icons_amount as f32 * (line_height + 4.0))
                    });
    ...
    fn draw(
        ...
        if let Some(label) = label.or_else(|| self.placeholder.clone()) {
            ...
            let icon_size = bounds.height - self.padding.top - self.padding.bottom - 2.0;
            let mut icon_offset = 0_f32;

            for svg in (self.svgs_from_option)(self.selected.as_ref().unwrap().borrow()) {
                let icon_bounds = Rectangle::new(
                    Point::new(bounds.x + self.padding.left * 0.7 + icon_offset, bounds.y + self.padding.top + 1.0),
            ...
            renderer.fill_text(
                ...
                Point::new(bounds.x + self.padding.left + icon_offset - 2.0, bounds.center_y()),
                if selected.is_some() {
                    style.text_color
                } else {
...
impl<'a, T, F, L, V, Message, Theme, Renderer>
From<IconPickList<'a, T, F, L, V, Message, Theme, Renderer>>
...
{
    fn from(
        pick_list: IconPickList<'a, T, F, L, V, Message, Theme, Renderer>,
    ) -> Self {
        Self::new(pick_list)
    }
}
