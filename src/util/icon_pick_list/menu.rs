...
use iced::{alignment, border, keyboard, touch, window, Background, Border, Color, Element, Event, Length, Padding, Pixels, Point, Rectangle, Shadow, Size, Theme, Vector};
...
impl<'a, 'b, T, F, Message, Theme, Renderer>
Menu<'a, 'b, T, F, Message, Theme, Renderer>
where
    T: ToString + Clone,
    F: Fn(&T) -> Vec<Svg> + 'a,
    Message: 'a,
    ...
    'b: 'a,
{
    ...
    pub fn new(
        ...
        Menu {
            state,
            options,
            svgs_from_option: determining_method,
            hovered_option,
...
where
    ...
    'b: 'a,
{
    pub fn new<T, F: Fn(&T) -> Vec<Svg> + 'a>(
        position: Point,
        viewport: Rectangle,
        menu: Menu<'a, 'b, T, F, Message, Theme, Renderer>,
        target_height: f32,
        menu_height: Length,
    ) -> Self
    where
...
struct List<'a, 'b, T, F, Message, Theme, Renderer>
where
    Theme: Catalog,
    Renderer: text::Renderer,
    F: Fn(&T) -> Vec<Svg>, // determining method
{
    options: &'a [T],
    svgs_from_option: F,
    hovered_option: &'a mut Option<usize>,
...
impl<T, F, Message, Theme, Renderer> Widget<Message, Theme, Renderer>
for List<'_, '_, T, F, Message, Theme, Renderer>
where
    T: Clone + ToString,
    F: Fn(&T) -> Vec<Svg>, // determining method
    Theme: Catalog,
    Renderer: text::Renderer + svg::Renderer,
{
    ...
    fn draw(
        ...
        for (i, option) in visible_options.iter().enumerate() {
            ...
            for svg in (self.svgs_from_option)(option) {
...
impl<'a, 'b, T, F, Message, Theme, Renderer>
From<List<'a, 'b, T, F, Message, Theme, Renderer>>
...
where
    ...
    'b: 'a,
{
    fn from(list: List<'a, 'b, T, F, Message, Theme, Renderer>) -> Self {
        Element::new(list)
    }
}
