//! Display tables.
//!
//! CUSTOM: Added to iced's some edge padding helpers and header only or content only alignments.
use iced_core;
    ...
    E: Into<Element<'a, Message, Theme, Renderer>>,
{
    Column {
        ...
        width: Length::Shrink,
        header_align_x: alignment::Horizontal::Left,
        header_align_y: alignment::Vertical::Top,
        content_align_x: alignment::Horizontal::Left,
        content_align_y: alignment::Vertical::Top,
    }
}

/// A grid-like visual representation of data distributed in columns and rows.
pub struct Table<'a, Message, Theme = iced_widget::Theme, Renderer = iced_renderer::Renderer>
where
    Theme: Catalog,
{
    columns: Vec<Column_>,
...
struct Column_ {
    width: Length,
    header_align_x: alignment::Horizontal,
    header_align_y: alignment::Vertical,
    content_align_x: alignment::Horizontal,
    content_align_y: alignment::Vertical,
}
    ...
    Renderer: iced_core::Renderer,
{
        ...
        let (mut columns, views): (Vec<_>, Vec<_>) = columns
            .map(|column| {
                ...
                (
                    Column_ {
                        width: column.width,
                        header_align_x: column.header_align_x,
                        header_align_y: column.header_align_y,
                        content_align_x: column.content_align_x,
                        content_align_y: column.content_align_y,
                    },
                    column.view,
                )
            })
    ...
    /// Sets if we add exception for the paddings on the edges of the [`Table`].
    pub fn edge_padding(mut self, edge: impl Into<EdgePadding>) -> Self {
        self.edge_padding = edge.into();
        self
    }
    ...
    Renderer: iced_core::Renderer,
{
    ...
    fn layout(
        ...
        let pad_top = if self.edge_padding.top {
            self.padding_y
        } else {
            0.0
        };
        let pad_right = if self.edge_padding.right {
            self.padding_x
        } else {
            0.0
        };
        let pad_bottom = if self.edge_padding.bottom {
            self.padding_y
        } else {
            0.0
        };
        let pad_left = if self.edge_padding.left {
            self.padding_x
        } else {
            0.0
        };
        ...
        let height_unit =
            (left.height - spacing_y * rows.saturating_sub(1) as f32 - pad_top - pad_bottom)
        ...
        for (i, cell) in cells.iter_mut().enumerate() {
            ...
            let (align_x, align_y) = if row == 0 {
                (header_align_x, header_align_y)
            } else {
        ...
        let intrinsic = limits.resolve(
            ...
            self.height,
            Size::new(
                x - spacing_x + pad_right,
                y + metrics
                    .rows
                    .last()
                    .copied()
                    .map(|height| height + pad_bottom)
                    .unwrap_or_default(),
            ),
        );
...
impl<'a, 'b, T, Message, Theme, Renderer> Column<'a, 'b, T, Message, Theme, Renderer> {
    ...
    /// Sets the alignment for the horizontal axis of the [`Column`]'s header.
    pub fn header_align_x(mut self, alignment: impl Into<alignment::Horizontal>) -> Self {
        self.header_align_x = alignment.into();
        self
    }

    /// Sets the alignment for the vertical axis of the [`Column`]'s header.
    pub fn header_align_y(mut self, alignment: impl Into<alignment::Vertical>) -> Self {
        self.header_align_y = alignment.into();
        self
    }

    /// Sets the alignment for the horizontal axis of the [`Column`]'s content.
    pub fn content_align_x(mut self, alignment: impl Into<alignment::Horizontal>) -> Self {
        self.content_align_x = alignment.into();
        self
    }

    /// Sets the alignment for the vertical axis of the [`Column`]'s content.
    pub fn content_align_y(mut self, alignment: impl Into<alignment::Vertical>) -> Self {
        self.content_align_y = alignment.into();
        self
    }
}

/// Whether the edge padding of a [`Table`] are enabled.
...
impl From<[bool; 2]> for EdgePadding {
    fn from(p: [bool; 2]) -> Self {
        Self {
            top: p[0],
            right: p[1],
            bottom: p[0],
