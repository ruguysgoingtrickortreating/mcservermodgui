...
//!
//! Custom: on_focus and on_blur messages.
//!
//! # Example
//! ```no_run
...
/// }
/// ```
pub struct TextInput<'a, Message, Theme = iced_core::Theme, Renderer = iced_widget::Renderer>
where
    Theme: Catalog,
    ...
    Renderer: text::Renderer,
{
    ...
    /// Sets the message that should be produced when the key [`key::Named::Escape`] is
    /// pressed.
                ...
                if let Some(focus) = &mut state.is_focused {
                    ...
                    match modified_key.as_ref() {
                        ...
                        keyboard::Key::Named(key::Named::Escape) => {
                            ...
                            if matches!(self.last_status, Some(Status::Focused { .. })) {
                                if let Some(on_escape) = self.on_escape.clone() {
                                    shell.publish(on_escape);
                                } else if let Some(on_blur) = self.on_blur.clone() {
                                    shell.publish(on_blur);
                                }
                            }
