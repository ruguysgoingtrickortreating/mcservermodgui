...
//!
//! CUSTOM: Support for None selection added
//!
//! # Example
//! ```no_run
...
{
    on_select: Box<dyn Fn(Option<T>) -> Message + 'a>,
...
impl<'a, T, L, V, Message, Theme, Renderer> PickListOption<'a, T, L, V, Message, Theme, Renderer>
where
    T: ToString + PartialEq + Clone,
    L: Borrow<[T]> + 'a,
    V: Borrow<T> + 'a,
    ...
    Renderer: text::Renderer,
{
    /// Creates a new [`PickListOption`] with the given list of options, the current
    ...
    /// Sets the placeholder of the [`PickListOption`].
    pub fn placeholder(mut self, placeholder: impl Into<String>) -> Self {
        self.placeholder = Some(placeholder.into());
        self
    }

    /// Sets the optional of the [`PickListOption`].
    pub fn optional(mut self, optional: bool) -> Self {
        self.optional = optional;
        self
    }

    /// Sets the None label of the [`PickListOption`].
    pub fn none_label(mut self, none_label: impl Into<String>) -> Self {
        self.none_label = none_label.into();
        self
    }

    /// Sets whether a separator is drawn between None and first option of the [`PickListOption`].
    pub fn none_separator(mut self, none_separator: bool) -> Self {
        self.none_separator = none_separator;
        self
    }

    /// Sets whether None is considered as a value and then would be shown as picked in the [`PickListOption`].
    pub fn none_is_value(mut self, none_is_value: bool) -> Self {
        self.none_is_value = none_is_value;
        self
    }

    /// Sets the width of the [`PickListOption`].
    ...
    /// Sets the [`Padding`] of the [`PickListOption`].
    pub fn padding<P: Into<Padding>>(mut self, padding: P) -> Self {
        self.padding = padding.into();
        self
    }

    /// Sets the text size of the [`PickListOption`].
    pub fn text_size(mut self, size: impl Into<Pixels>) -> Self {
        self.text_size = Some(size.into());
        self
    }

    /// Sets the text [`text::LineHeight`] of the [`PickListOption`].
    pub fn text_line_height(mut self, line_height: impl Into<text::LineHeight>) -> Self {
        self.text_line_height = line_height.into();
        self
    }

    /// Sets the [`text::Shaping`] strategy of the [`PickListOption`].
    pub fn text_shaping(mut self, shaping: text::Shaping) -> Self {
        self.text_shaping = shaping;
        self
    }

    /// Sets the font of the [`PickListOption`].
    pub fn font(mut self, font: impl Into<Renderer::Font>) -> Self {
        self.font = Some(font.into());
        self
    }

    /// Sets the [`Handle`] of the [`PickListOption`].
    pub fn handle(mut self, handle: Handle<Renderer::Font>) -> Self {
        self.handle = handle;
        self
    }

    /// Sets the message that will be produced when the [`PickListOption`] is opened.
    pub fn on_open(mut self, on_open: Message) -> Self {
        self.on_open = Some(on_open);
        self
    }

    /// Sets the message that will be produced when the [`PickListOption`] is closed.
    pub fn on_close(mut self, on_close: Message) -> Self {
        self.on_close = Some(on_close);
        self
    }

    /// Sets the style of the [`PickListOption`].
    #[must_use]
    ...
    /// Sets the style class of the [`PickListOption`].
...
impl<'a, T, L, V, Message, Theme, Renderer> Widget<Message, Theme, Renderer>
    for PickListOption<'a, T, L, V, Message, Theme, Renderer>
where
    ...
    Renderer: text::Renderer + 'a,
{
        ...
        match event {
            ...
            Event::Mouse(mouse::Event::WheelScrolled {
                    ...
                    && !state.is_open
                {
                    ...
                    if let Some(next_option) = next_option.cloned() {
                        shell.publish((self.on_select)(Some(next_option)));
                    }
    ...
    fn draw(
        ...
        let (label, color) = if let Some(label) = label {
            (Some(label), style.text_color)
        } else if self.none_is_value && !self.none_label.is_empty() {
            (Some(self.none_label.clone()), style.text_color)
        } else if let Some(placeholder) = self.placeholder.clone() {
            (Some(placeholder), style.placeholder_color)
        } else {
            (None, style.placeholder_color)
        };
...
impl<'a, T, L, V, Message, Theme, Renderer>
    From<PickListOption<'a, T, L, V, Message, Theme, Renderer>>
...
where
    ...
    Renderer: text::Renderer + 'a,
{
    fn from(pick_list: PickListOption<'a, T, L, V, Message, Theme, Renderer>) -> Self {
        Self::new(pick_list)
    }
}
...
impl<P: text::Paragraph> State<P> {
    /// Creates a new [`State`] for a [`PickListOption`].
    fn new() -> Self {
        Self {
...
/// The handle to the right side of the [`PickListOption`].
...
pub enum Handle<Font> {
    ...
    Dynamic {
        /// The [`Icon`] used when [`PickListOption`] is closed.
        closed: Icon<Font>,
        /// The [`Icon`] used when [`PickListOption`] is open.
        open: Icon<Font>,
    },
...
/// The possible status of a [`PickListOption`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Status {
    /// The [`PickListOption`] can be interacted with.
    Active,
    /// The [`PickListOption`] is being hovered.
    Hovered,
    /// The [`PickListOption`] is open.
    Opened {
        /// Whether the [`PickListOption`] is hovered, while open.
        is_hovered: bool,
    },
}
...
/// The theme catalog of a [`PickListOption`].
pub trait Catalog: menu::Catalog {
    ...
    /// The default class for the menu of the [`PickListOption`].
...
/// A styling function for a [`PickListOption`].
///
...
/// The default style of the field of a [`PickListOption`].
...
/// The default [`Padding`] of a [`PickListOption`].
