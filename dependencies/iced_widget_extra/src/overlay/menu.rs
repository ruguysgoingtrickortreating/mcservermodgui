...
pub struct Menu<'a, 'b, T, Message, Theme = iced_core::Theme, Renderer = iced_widget::Renderer>
where
    Theme: Catalog,
    ...
    Renderer: text::Renderer,
{
    fn handle_option_selection(&mut self, index: usize, shell: &mut Shell<'_, Message>) {
        if self.optional && index == 0 {
            shell.publish((self.on_selected)(None));
        } else if let Some(option) = self.options.get(index - if self.optional { 1 } else { 0 }) {
            shell.publish((self.on_selected)(Some(option.clone())));
        }
    }
