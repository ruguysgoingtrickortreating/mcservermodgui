...
//!
//! Custom: on_focus, on_blur, on_escape messages.
//!
//! # Example
//! ```no_run
...
use iced_core::{
    ...
    Rectangle, Shell, Size, SmolStr, Theme, Vector, theme,
};
...
pub struct Content<R = iced_widget::Renderer>(RefCell<Internal<R>>)
where
    R: text::Renderer;
...
/// Produces a [`Task`] that returns whether the [`TextEditor`] with the given [`Id`] is focused or not.
...
pub fn focus<T>(id: impl Into<widget::Id>) -> Task<T> {
    task::effect(iced_runtime::Action::widget(operation::focusable::focus(
        id.into(),
    )))
}
            ...
            match update {
                Update::Click(click) => {
                    ...
                    if let Some(on_focus) = self.on_focus.clone()
                        && !matches!(self.last_status, Some(Status::Focused { .. }))
                    {
                ...
                Update::Binding(binding) => {
                    fn apply_binding<H: text::Highlighter, R: text::Renderer, Message: Clone>(
                        ...
                        match binding {
                            Binding::Unfocus => {
                                if let Some(on_blur) = on_blur
                                    && matches!(last_status, Some(Status::Focused { .. }))
                                {
                            ...
                            Binding::Escape => {
                                if matches!(last_status, Some(Status::Focused { .. }))
                                    && let Some(on_blur) = on_blur
                                {
                                    shell.publish(on_blur.clone());
                                }
...
pub enum Binding<Message> {
    ...
    Insert(char),
    /// Unfocus the [`TextEditor`], but with user input.
    Escape,
    /// Break the current line, or submit if on_submit is Some.
    Enter,
